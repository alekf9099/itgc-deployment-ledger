...
async function accessToken() {
  ...
  /* 키가 PEM 형식이어도 내용이 잘렸으면 여기서 실패합니다. */
  let signature;
  try {
    signature = signer.sign(pem);
  } catch (e) {
    throw new Error(
      'GOOGLE_SA_PRIVATE_KEY 로 서명하지 못했습니다. 값이 잘렸거나 다른 키일 수 있습니다. ' +
      '키 JSON 을 다시 내려받아 private_key 값을 그대로 넣으십시오. ' +
      `(원인: ${e.message})`
    );
  }
 ...
 * 범위 하나를 읽습니다.
 *
 * raw=true 면 표시 형식이 아니라 원래 값을 받습니다. 날짜가 일련값(숫자)으로
 * 오므로 표시 형식(지역 설정)에 따라 결과가 달라지지 않습니다.
 */
export function readRange(range, raw = false) {
  const q = raw ? '?valueRenderOption=UNFORMATTED_VALUE' : '';
  return call(`/values/${encodeURIComponent(range)}${q}`);
}
