...
function readToken(token) {
  ...
  if (!userId || !exp || Number(exp) < Math.floor(Date.now() / 1000)) return null;
...
export async function currentUser(req) {
  const userId = readToken(parseCookies(req)[COOKIE]);
  if (!userId) return null;
  return one(
    `SELECT id, username, name, unit, role, active FROM users WHERE id = $1 AND active`,
    [userId]
  );
}
