 ...
 * 증적 문서 ID 형식
 *
 * 현재 체계는 `QA-{배포일 YYYYMMDD}-{일련 2자리}` 입니다. 유형은 ID 가 아니라
 * 「릴리즈 구분」 열로만 관리합니다.
 *
...
export function judge(r) {
  ...
  if (!r.id || !r.date) v.idc = null;
  else {
    const parsed = parseDocId(r.id);
    if (!parsed) v.idc = 'bad';                                   // 형식 위반
    else if (parsed.date !== ymd(r.date)) v.idc = 'bad';           // 날짜부 불일치
    /* 구 형식의 유형코드는 남아 있는 값이므로 그대로 검사합니다. */
    else if (parsed.code && (!r.type || parsed.code !== CODE[r.type])) v.idc = 'bad';
    else v.idc = 'ok';
  }
...
export const CHECK_DEFS = [
  ...
  { key: 'idc', name: '증적 문서 ID 정합성 불일치', ctrl: 'PC-01', isDef: true,
    crit: 'ID 형식이 QA-{배포일 YYYYMMDD}-{일련 2자리} 가 아니거나 날짜부가 배포일과 불일치. 부여 오류이므로 사유를 확인합니다.' },
