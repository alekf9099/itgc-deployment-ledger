# QA 검증 이력 대장 (ITGC PC-01 / PD-02)

QA유닛이 운영하는 QA 검증 이력 대장입니다. 배포 이력 전수를 모집단으로 기재하고,
...
| 초기 비밀번호 | 발급·재설정한 비밀번호는 **최초 로그인 시 변경을 강제**합니다. 관리자가 아는 값으로 계속 쓰이면 그 계정의 기록이 본인 것이라고 단정할 수 없습니다. |
...
| 세션 관리 | HttpOnly·Secure·SameSite 쿠키, 12시간 만료. **비밀번호 변경·재설정 시 그 계정의 기존 세션이 모두 해제됩니다.** |
| 시도 제한 | 15분 동안 로그인 5회 실패 시 차단하고 차단 사실을 기록합니다. |
| 동시 수정 | 불러온 시점 이후 다른 담당자가 저장했으면 거부합니다. 조용한 덮어쓰기가 없습니다. |
| 점검 회차 | 대상 기간당 한 건만 확정됩니다. 재작성 시 기존 기록은 제외 표시로 보존됩니다. |
| 반출본 무결성 | 파일과 SHA-256 을 서버가 만들어 함께 기록합니다. 보관본이 원본과 같은지 확인할 수 있습니다. |
...
| `api/checks.js` | 월간 점검 회차 (집계·작성중·확정·재작성) |
| `api/audit.js` | 변경 이력 조회 (조회 전용) |
| `api/users.js` | 계정 발급·권한 변경·비활성화·비밀번호 재설정 (admin 전용) |
| `api/storage.js` | 용량 통계·반출 주기 공지 |
| `api/export.js` | 증적 반출 (CSV 생성 · SHA-256 기록) |
| `api/sheet.js` | Google Sheets 반사 (웹 → 시트) |
| `api/sheet-import.js` | 시트에서 가져오기 (시트 → 웹, 확인 후 적용) |
...
```bash
POSTGRES_URL="복사한_연결문자열" node scripts/create-user.mjs hhkim 김홍현 admin QA유닛 [비밀번호]
```
...
| 상황 | 조치 |
|---|---|
| 신규 담당자 | 계정 관리 → 계정 추가. 비밀번호를 직접 정하거나 비워서 자동 생성. 한 번만 표시됩니다 |
| 비밀번호 분실 | 계정 관리 → **비밀번호 재설정**. 직접 정하거나 비우면 자동 생성 |
...
> **정본은 대장입니다.** 시트에서 고친 값은 다음 반사에서 덮어써집니다.
> 시트 내용을 대장에 넣으려면 **「시트에서 가져오기」로 확인 후 적용**해야
> 합니다 (아래 참조). 자동 양방향 동기화는 만들지 않았습니다 — 정본이 둘이
> 되면 어느 쪽이 기준인지 답할 수 없습니다.
...
코드가 시트 탭 이름으로 범위를 지정합니다 (`'배포관리대장'!A6:AB1000`).
...
**문서 이름은 「QA 검증 이력 대장」으로 바뀌었지만 탭 이름은 그대로입니다.**
탭 이름을 바꾸면 반사·반입이 「머리글이 양식과 다릅니다」로 중단됩니다.
바꿔야 한다면 `lib/sheet.js` 의 `LEDGER_SHEET` · `CHECK_SHEET` 도 함께 고치고
검사(`scripts/test-sheet.mjs` · `test-import.mjs`)를 갱신해야 합니다.
...
시트에 입력한 내용을 대장으로 반입합니다. **자동 동기화가 아니라, 눌러서
미리보기를 확인하고 적용하는 절차**입니다.
...
| 규칙 | 이유 |
|---|---|
| 증적 문서 ID 로 대장 건을 찾음 | ID 가 없는 행은 식별할 수 없어 제외 |
| 필수 항목·날짜 형식·허용값 검증 | 시트에는 입력 검증이 없어 아무 값이나 들어올 수 있음 |
| **웹에서 먼저 수정된 건은 건너뜀** | 조용한 덮어쓰기를 만들지 않음 |
| 시트에서 지운 행은 무시 | 대장에서 건을 빼는 것은 웹에서 관리자만 |
| 판정 열은 가져오지 않음 | 시트에서 판정을 고칠 수 있으면 통제가 무의미 |
| **적용은 admin 만** | 모집단을 바꾸는 행위 |
| 적용을 누른 사람이 수행자로 기록 | 시트 편집자는 특정할 수 없음 |

적용 시점에 계획을 **다시 계산**합니다. 화면이 보낸 목록을 그대로 저장하면
화면에서 값을 바꿔 보낼 수 있기 때문입니다.
...
| 증적 반출 이력 | 수행자·일시·범위·건수·**SHA-256** (CSV 내보내기 시 자동 기록) |
